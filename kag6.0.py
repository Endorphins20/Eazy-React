...
class PlannerPrompt(BasePrompt):  # REFINED: For deeper recursive-like planning
    ...
    **核心指令与思考链 (Chain-of-Thought for Reflective Planning):**
    ...
    2.  **审视历史 (`${task_history}`)**: 
        ...
        * **【最关键决策点】**: 最近的 `DeduceExecutor` 步骤的结果中，`is_sufficient` 是否为 `false`？并且，其 `new_questions_or_entities` 列表中是否包含对【特定标准、文件编号、或实体详情】的查询请求（例如，“查询通则1107的详细内容”、“查询通则1101的详细内容”）？
    ...
    4.  **决策制定 (`final_thought` 和 `plan_status`)**:
        * **已解决?** 如果所有子问题都已解决，信息充分，并且所有在推理中提及并被标记为需要查询其细节的【关键标准/文件（如“通则0104”，“通则1107”）都已成功检索到其详细内容，并被后续的Deduce步骤分析和确认信息充分】，则规划一个 `FinishAction` 任务，并将 `plan_status: "finished"`。
...
class DeducePrompt(BasePrompt):  # REFINED: Stronger guidance for new_questions
	# In class DeducePrompt:
	SYSTEM_PROMPT = """
	    你是一位严谨的AI推理专家。你的任务是根据提供的“上下文信息”，精确地回答或完成“推理目标”。
	    你必须特别注意上下文中任何提及的【具体标准、规范、文件编号或专有名词】（例如“通则0104”、“XYZ协议”、“ABC操作手册”、“某某法规第Y条”等）。

	    请严格按照以下JSON格式输出你的结论和评估：
	    ```json
	    {
	      "answer_summary": "string (对推理目标的直接、简洁的回答或总结。如果信息不足，明确说明当前已知什么，并【清晰指出具体还缺少哪些信息或哪些被引用的标准/文件细节】才能完整回答推理目标)",
	      "is_sufficient": boolean (true 如果你认为提供的上下文信息【已包含所有必要的细节，特别是所有被明确引用或提及的标准/文件的具体内容，无需任何补充查询】来完全回答推理目标，否则 false),
	      "new_questions_or_entities": [
	        "string" // 【至关重要指令】如果 is_sufficient 为 false，这里【必须】列出：
	                  // 1. 为了获得【缺失的关键细节】需要查询的具体问题。
	                  // 2. 如果上下文中明确引用了某个【标准、文件编号或特定实体名称】（例如“详见通则1107”、“依据GB/T标准X”），但其【具体内容（例如限值、方法、条款）并未在当前提供给你的上下文中详细列出】，则【必须】将查询该标准/文件/实体的具体内容（例如“查询《中国药典》通则1107中关于微生物限度的具体标准是什么？”或“查找GB/T标准X的详细文本”）作为条目加入此列表。
	                  // 确保列表中的每一项都是一个明确的、可用于下一步检索的查询目标。如果信息完全充分，则为空列表[]。
	      ]
	    }
	    ```
...
async def run_main_logic_with_user_data_deep_recursive():
	...
	pipeline = IterativePipeline(planner, executors_map, generator,
								 max_iterations=8)  # Allow more iterations for deep recursion
